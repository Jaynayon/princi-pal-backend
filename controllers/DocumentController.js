...
async function getLrByYearMonth(req, res) {
    try {
...
async function getRcdByYearMonth(req, res) {
    try {
        const document = await Document.findOne({
            school: res.school._id,
            year: req.params.year,
        ...
        const rcdEntries = await LR.find({ document: document._id }) //we need the id to be used as keys. 
        ...
        //get the sum on the amount property
        const amountSum = rcdEntries.reduce((total, lr) => total + lr.amount, 0);
...
async function getJevByYearMonth(req, res) {
    try {
        const document = await Document.findOne({
            school: res.school._id,
            year: req.params.year,
...
async function searchLr(req, res) {
    try {
